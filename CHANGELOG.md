...
- `/check`, `/checkall`, the menu **🔍 Check all** button and the per-product
  ...
  `chart` for the photo path). Phase 1 F1 left **seven** deferred imports of
...
- The `/add <url>` flow crashed in production with
  ...
  the bot. A handler-side audit revealed that the Phase 1 F1 monolith
...
- `/add <url>` and CSV import (`/import`) crashed with
  ...
  `ScraperRegistry` was being called like an individual scraper. Phase 1
